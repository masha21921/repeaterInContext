...
import { ButtonSettingsPanel } from '../components/ButtonSettingsPanel';
...
function createComponent(type) {
  ...
  if (type === 'textInput') { comp.value = ''; comp.role = ''; comp.boundField = ''; comp.boundContext = ''; }
  if (type === 'dropdown') { comp.value = ''; comp.role = ''; comp.boundField = ''; comp.boundContext = ''; }
  if (type === 'button') { comp.label = 'Submit'; comp.action = ''; comp.boundContext = ''; }
  return comp;
}

const INPUT_ADDABLE_COMPONENTS = [
  { type: 'textInput', label: 'Text Input' },
  { type: 'dropdown', label: 'Dropdown' },
  { type: 'button', label: 'Button' },
];

export function InputComponentsEditor() {
  ...
  const contextInstanceLabelMap = useMemo(() => {
    ...
    (sectionContextIds.section1 || []).forEach((id) => assign(id, `section1-${id}`));
    (sectionContextIds.section2 || []).forEach((id) => assign(id, `section2-${id}`));
    (sectionContextIds.section3 || []).forEach((id) => assign(id, `section3-${id}`));
    const sectionList = [
    ...
    sectionList.forEach(([sectionId, comps]) => {
      comps.forEach((comp) => {
        ...
        const parentCtxIds = sectionContextIds[sectionId] || [];
        if (parentCtxIds.includes(comp.assignedContextId) || comp.assignedContextId === pageContextId) {
          const parentKey = parentCtxIds.includes(comp.assignedContextId) ? `${sectionId}-${comp.assignedContextId}` : 'page';
  ...
  const handleAddComponentFromContext = useCallback((sectionId, contextId, fieldOrAction, suggestedType) => {
    const setter = getSetBySection(sectionId);
    setter((prev) => {
      const comp = createComponent(suggestedType);
      comp.boundContext = contextId;
      
      if (suggestedType === 'textInput' || suggestedType === 'dropdown') {
        if (fieldOrAction === 'setFilter') {
          comp.role = 'filter';
          comp.boundField = 'brand'; // default assumption for filter
        } else {
          comp.boundField = fieldOrAction;
          comp.role = 'update'; // default assumption for data fields
        }
      } else if (suggestedType === 'button') {
        comp.action = fieldOrAction === 'updateCurrentItem' ? 'submitUpdate' : '';
      } else if (suggestedType === 'image') {
        comp.boundFieldImage = fieldOrAction;
      }
  ...
  const renderComponent = useCallback((comp, sectionId) => {
    ...
    if (comp.type === 'textInput') {
      const isSelected = selected?.type === 'component' && selected?.sectionId === sectionId && selected?.componentId === comp.id;
      return (
        <div className="component-with-toolbar">
          {isSelected && (
            <RepeaterFloatingToolbar
          ...
          <TextInput
            ...
            onChange={(val) => {
              const setter = getSetBySection(sectionId);
              setter((prev) => prev.map((c) => (c.id === comp.id ? { ...c, value: val } : c)));
            }}
          />
        </div>
      );
    ...
    if (comp.type === 'dropdown') {
      ...
      // Generate options based on component configuration
      let displayOptions = ['Option 1', 'Option 2', 'Option 3'];
      
      if (comp.optionsSourceType === 'static' && comp.staticOptions) {
        displayOptions = comp.staticOptions.split(',').map(opt => opt.trim()).filter(opt => opt);
      } else if (comp.optionsSourceType === 'dynamic' || !comp.optionsSourceType) {
        if (comp.optionsSource === 'other' && comp.optionsContext && comp.optionsField) {
          // Show sample values from the specified field
          const sampleData = {
          ...
          displayOptions = sampleData[comp.optionsContext]?.[comp.optionsField] || ['Option 1', 'Option 2', 'Option 3'];
        } else if (comp.boundContext && comp.boundField) {
          // Show unique values from the target field
          const sampleData = {
            'recipes': { title: ['Pasta Carbonara', 'Caesar Salad', 'Tiramisu'], course: ['Appetizer', 'Main', 'Dessert'], description: ['Classic Italian dish', 'Fresh greens', 'Coffee-flavored dessert'] },
            'team': { title: ['Alice Johnson', 'Bob Smith', 'Carol Davis'], role: ['Designer', 'Developer', 'Manager'], image: ['photo1.jpg', 'photo2.jpg', 'photo3.jpg'] },
            'services': { title: ['Web Design', 'Branding', 'SEO'], duration: ['2 weeks', '4 weeks', '8 weeks'], description: ['Professional design', 'Full brand identity', 'Search optimization'] },
          ...
          displayOptions = sampleData[comp.boundContext]?.[comp.boundField] || ['Option 1', 'Option 2', 'Option 3'];
        }
      }
  ...
  function renderSettingsPanel() {
    ...
    if (selected.type === 'page') {
      ...
      const attachedContexts = pageContextId ? [{
        contextId: pageContextId,
        label: pageCtx?.label ?? '—',
        instanceLabel: contextInstanceLabelMap['page'] ?? '—',
        settings: { pageLoad: 4, filterRules: [], sortRules: [] },
        sortSummary: 'Default',
      }] : [];
      ...
      return (
        <PageSettingsPanel
          ...
          onDisconnectContext={() => updatePageContext(null)}
        />
      );
    }
    if (selected.type === 'section') {
      const sectionId = selected.sectionId;
      const contextIds = sectionContextIds[sectionId] || [];
      const attachedContexts = contextIds.map((contextId) => ({
        contextId,
        label: getContextById(contextId)?.label ?? '—',
      ...
      return (
        <SectionSettingsPanel
          sectionId={sectionId}
          attachedContexts={attachedContexts}
          allComponents={[...section1Components, ...section2Components, ...section3Components]}
          onOpenConnectModal={() => setConnectContextTarget({ type: 'section', sectionId })}
          onClose={() => setSelected(null)}
          onDisconnectContext={(contextId) => removeSectionContext(sectionId, contextId)}
          onAddComponentFromContext={(contextId, fieldOrAction, suggestedType) => handleAddComponentFromContext(sectionId, contextId, fieldOrAction, suggestedType)}
        />
      );
    }
    ...
    if (selected.type === 'component' && selectedRepeaterComp?.type === 'textInput') {
      return (
        <InputSettingsPanel
          component={selectedRepeaterComp}
          availableContexts={availableContextsForRepeater}
          onChange={(updates) => {
            const setter = getSetBySection(selected.sectionId);
            setter((prev) => prev.map((c) => (c.id === selected.componentId ? { ...c, ...updates } : c)));
          }}
          onClose={() => setSelected(null)}
        />
      );
    }
    if (selected.type === 'component' && selectedRepeaterComp?.type === 'dropdown') {
      return (
        <InputSettingsPanel
          component={selectedRepeaterComp}
          availableContexts={availableContextsForRepeater}
    ...
    if (selected.type === 'component' && selectedRepeaterComp?.type === 'button') {
      return (
        <ButtonSettingsPanel
          component={selectedRepeaterComp}
          availableContexts={availableContextsForRepeater}
  ...
  return (
    <div className="editor editor--input-components">
      ...
      <div className="editor-workspace">
        <section className="editor-canvas-section">
          <Stage
            className="stage--input-components"
            isPageSelected={selected?.type === 'page'}
            onSelectPage={selectPage}
            showPageHeader={true}
            pageContexts={pageContextId ? [{
              label: getContextById(pageContextId)?.label ?? '—',
              instanceLabel: contextInstanceLabelMap['page'] ?? '—',
            }] : []}
            onOpenPageConnect={() => setConnectContextTarget({ type: 'page' })}
            showPageConnectButton={true}
            hasPageContext={!!pageContextId}
          >
            <Section
              sectionId="section1"
              ...
              addableComponents={INPUT_ADDABLE_COMPONENTS}
              ...
              onOpenSectionConnect={() => setConnectContextTarget({ type: 'section', sectionId: 'section1' })}
            ...
            <Section
              ...
              onOpenSectionConnect={() => setConnectContextTarget({ type: 'section', sectionId: 'section2' })}
              showSectionConnectButton={true}
              showContextLabel={true}
            />
            <Section
              ...
              onOpenSectionConnect={() => setConnectContextTarget({ type: 'section', sectionId: 'section3' })}
              showSectionConnectButton={true}
              showContextLabel={true}
            />
          </Stage>
